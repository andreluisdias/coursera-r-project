...
rankall <- function(outcome, num = "best") {
        ...
        all_states <- sort(unique(outcome_csv$State))
        
        mainResult <- NA
                
        for (stateIdx in 1:length(all_states)) {
                state <- all_states[stateIdx]
                ...
                if(!is.na(finalIdx)) {
                        ...
                        hospitaldf <- data.frame(cbind(as.character(hospital.found), state))
                        
                        if (is.na(mainResult)) {
                                mainResult <- hospitaldf
                        } else {
                                mainResult <- rbind(mainResult, hospitaldf)
                        }

                } else {
                        hospitalnadf <- cbind(NA, state)
                        
                        if (is.na(mainResult)) {
                                mainResult <- hospitalnadf
                        } else {
                                mainResult <- rbind(mainResult, hospitalnadf)
                        }                        
                }
        }
        
        names(mainResult) <- c("hospital", "state")
        return(mainResult)
}
